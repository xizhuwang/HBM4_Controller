...
async function loadTs(path) {
  ...
  if (path.endsWith('challenges.ts')) {
    ...
    const extensionJs = ts.transpileModule(extensionSource, { compilerOptions: { module: ts.ModuleKind.ESNext, target: ts.ScriptTarget.ES2022 } }).outputText;
...
verify(challenges.length === 40 && new Set(challenges.map((c) => c.id)).size === 40, '40 unique bilingual challenges');
