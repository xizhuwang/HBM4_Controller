...
const storageKeys = { locale: 'memory-controller-lab:locale', solved: 'memory-controller-lab:solved', code: 'memory-controller-lab:solutions' };
...
const copy = {
  zh: {
    subtitle: 'HBM、DRAM、LPDDR、GDDR 與 PCIe Controller 實作訓練', search: '搜尋題目', tracks: '學習路徑', all: '全部題目',
    ...
    testGroups: '檢查項目', next: '下一題', codeLocal: '程式與進度儲存在此瀏覽器；勿貼上機密或個資', independent: '本站為獨立的記憶體與高速介面 Controller RTL 教學專案。',
  ...
  en: {
    subtitle: 'Hands-on HBM, DRAM, LPDDR, GDDR and PCIe controller RTL training', search: 'Search challenges', tracks: 'Learning tracks', all: 'All challenges',
    ...
    testGroups: 'Test groups', next: 'Next challenge', codeLocal: 'Saved in this browser; do not paste confidential or personal data', independent: 'An independent hands-on learning project for memory and high-speed interface controller RTL.',
...
function rankFor(points: number, locale: Locale) {
  const ranks: [number, string][] = locale === 'zh'
    ? [[0, 'RTL 新手'], [600, 'Controller Builder'], [1600, 'CDC／AXI Debugger'], [3000, 'Memory Scheduler Designer'], [5000, 'Sign-off Candidate']]
    : [[0, 'RTL Starter'], [600, 'Controller Builder'], [1600, 'CDC/AXI Debugger'], [3000, 'Memory Scheduler Designer'], [5000, 'Sign-off Candidate']];
...
export default function Home() {
  ...
  const context = learningContext[current.id] ?? {
    why: { zh: 'Controller 的正確性來自明確狀態、時序限制與可重現驗證；本題把一個真實設計責任縮小成可觀察的 RTL 契約。', en: 'Controller correctness comes from explicit state, timing constraints, and reproducible verification; this lab reduces one real design responsibility to an observable RTL contract.' },
    roles: { zh: 'Memory Controller RTL、Interface IP、Design Verification', en: 'Memory Controller RTL, Interface IP, Design Verification' },
  };
  ...
  useEffect(() => {
    ...
    nav.modelContext.registerTool({
      name: 'open_memory_controller_challenge', description: 'Open one Memory Controller RTL Lab challenge by its id.',
    ...
    return () => nav.modelContext?.unregisterTool?.('open_memory_controller_challenge');
  }, [selectChallenge]);
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      <header className="sticky top-0 z-30 border-b border-border bg-card/95 backdrop-blur">
        <div className="mx-auto flex max-w-[1580px] items-center justify-between gap-4 px-4 py-3 sm:px-6">
          <button type="button" className="flex items-center gap-3 text-left" onClick={() => selectChallenge(challenges[0].id)}>
            ...
            <div><div className="flex items-baseline gap-2"><h1 className="font-mono text-base font-semibold tracking-tight">Memory Controller RTL Lab</h1><span className="text-xs text-muted-foreground">by Xi-Zhu Wang</span></div><p className="hidden text-xs text-muted-foreground sm:block">{text.subtitle}</p></div>
          </button>
          <div className="flex items-center gap-2">
            <a href="https://github.com/xizhuwang/HBM4_Controller" target="_blank" rel="noreferrer"><Button variant="ghost" size="sm" className="hidden sm:inline-flex">{text.source}<ExternalLink /></Button></a>
      ...
      <footer className="border-t border-border bg-card px-5 py-6 text-center text-sm leading-6 text-muted-foreground">
        ...
        <p>© 2026 Xi-Zhu Wang · <a className="underline" href="./LICENSE-GPL-3.0.txt">GPL-3.0-or-later</a> · {locale === 'zh' ? '開源教學版本，不提供正確性或適用性保證' : 'Open-source educational edition; without warranty'}</p>
